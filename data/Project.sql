...
select distinct condition_concept_id, concept_name,
count(*) as CONDITION_COUNT from condition_occurrence, concept
where condition_concept_id=concept.concept_id and
concept_name like '%diabete%'
group by condition_concept_id order by CONDITION_COUNT desc;
